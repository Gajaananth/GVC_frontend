...
const Modal = ({ title, onClose, children, wide, className }: ModalProps) => (
  <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-2 md:p-4 bg-black/50 backdrop-blur-sm">
    <div className={`bg-white rounded-t-3xl sm:rounded-2xl shadow-2xl w-full ${wide ? 'sm:max-w-4xl md:max-w-5xl' : 'sm:max-w-2xl'} ${className ?? ''} max-h-[85vh] sm:max-h-[90vh] flex flex-col min-w-0 sm:my-4`}>
